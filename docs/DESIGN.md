...
#### `data-scope` — the component boundary (added)

The three helpers above scan a whole subtree. That is right for a single view and wrong the moment components **nest** (a dialog holding tabs): the parent's `refs` picks up the child's `data-ref` names, `applyBindings` writes the parent's data into the child, and a `data-on` action shared by both fires twice (events bubble, and the child's node passes the parent's `root.contains` guard — the old §6 limitation). The fix is one rule, not a registry: **an element belongs to its nearest ancestor-or-self carrying `data-scope`**, and each helper only sees elements whose scope is its own `root` or lies outside it. A nested root marked `data-scope`, and everything under it, is invisible to the parent's helpers. Implemented as a five-line predicate (`inScope`) applied in the three helpers.

Why this shape: it is **explicit** (the boundary is an attribute the author writes on the template root — P1, P2), **native** (`closest` + `contains` — P3), **inert by default** (no `data-scope`, no change — full backward compatibility), and **tiny** (P5). We deliberately do _not_ have `createView`/`mount` stamp the attribute automatically — that would be hidden machinery deciding boundaries the author can't see. The attribute value is optional; a name (`data-scope="dialog"`) reads well in devtools and doubles as a CSS `@scope` anchor, which is a pleasant coincidence rather than a coupling.
...
- **`data-scope`** on a component's template root whenever components nest. Sibling components don't need it; nested ones do (§4.5).
...
- **Nested roots share a namespace unless you mark them.** Without `data-scope`, `refs` / `applyBindings` / `delegate` scan the whole subtree, so a nested parent and child that share a `data-ref` name or a `data-on` action collide (both delegates fire — events bubble and the child's node passes `root.contains`). Sibling components (the common case — §4.6) never collide; for nested components, put `data-scope` on the child's root (§4.5) — the parent's helpers then stop at that boundary. This is opt-in by design: the library does not guess where components begin.
...
- **scope (`data-scope`)** — the component boundary: an element belongs to its nearest ancestor-or-self carrying `data-scope`; `refs` / `applyBindings` / `delegate` do not see into a nested scope (§4.5).
